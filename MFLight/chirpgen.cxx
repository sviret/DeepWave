...
void chirpgen::create_function()
{
    ...
    Then when the power is chosen we choose a random starting phase Phi0 in order to make sure that the frequency component is fully randomized.
    */
    ...
    for (int i = 0; i<T->size()/2; i++)
    {
      ...
      double phi0 = float(rand()%1000)/1000.*8*atan(1.);
        
      // The noise is random, so is the initial phase for each frequency
        
      input[i][0] *= cos(phi0);
      input[i][1] *= sin(phi0);
        
      // For the plot to correspond to the real expected value we retrieve the
      // normalization factor
      Nfr->push_back(input[i][0]/sqrt(n_size));
      Nfi->push_back(input[i][1]/sqrt(n_size));
      Tf->push_back(float(i)/n_size*f_s*1000);
    ...
    // We don't enter the negative frequency coefficients, so
