...
class GenTemplate:
    ...
    def __init__(self,Tsample=1,fe=2048,fDmin=15,fDmax=1000,kindTemplate='EM',whitening=1,verbose=False):
    ...
    def _whitening(self,kindPSD,Tsample,norm):
        ...
        Noise=gn.GenNoise(Ttot=self.__Ttot,fe=self.__fe, kindPSD=kindPSD,fmin=self.__fDmin,fmax=self.__fDmaxd,whitening=self.__whiten)
        Noise.getNewSample()
        self.Noise=Noise
    ...
    def getNewSample(self,kindPSD='flat',Tsample=1,tc=None,norm=True):
        ...
        #print(itc,len(S),len(self.__St),Tsample)
        if tc<=self.__Ttot:
