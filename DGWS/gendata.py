...
class GenDataSet:
    ...
    def __init__(self,mint=(10,50),NbB=1,tcint=(0.75,0.95),kindPSD='flat',kindTemplate='EM',Ttot=1,fe=2048,kindBank='linear',paramFile=None,step=0.1,choice='train',length=100,whitening=1):
        ...
        if self.__choice!='frame':
            ...
            self.__NGenerator=gn.GenNoise(Ttot=self.__listTtot,fe=self.__listfe,kindPSD=self.__kindPSD,whitening=self.__whiten)
            # Template stream generator
            self.__TGenerator=gt.GenTemplate(Tsample=self.__listTtot,fe=self.__listfe,kindTemplate=self.__kindTemplate,whitening=self.__whiten)
    ...
    def _genNoiseSequence(self):
        ...
        for i in range(nsamples):
            ...
            chunck.append(self.__NGenerator.getNoise())
           
        self.__Noise.append(npy.ravel(npy.squeeze(chunck)))
    ...
    def saveGenerator(self,dossier):
        ...
        for k in range(self.__nTtot):
            fname=dossier+self.__choice+'-'+str(self.__nTtot)+'chunk'+'-'+self.__kindPSD+'-'+self.__kindTemplate+'-'+self.__kindBank+'-'+str(k)+'-'+str(self.__whiten)+'-'+str(self.__Ttot)+'s'+'-data'
    ...
    def saveFrame(self,dossier):
        ...
        npy.savez_compressed(fname,self.__Noise[0])
        self.__listfnames.append(fname)
...
def main():
    ...
    if args.cmd=='noise': # Simple Noise generation
        ...
        TGenerator=gt.GenTemplate(Tsample=args.time,fe=args.fe,kindTemplate=args.kindTemplate,fDmin=args.fmin,fDmax=args.fmax,whitening=args.white,verbose=args.verbose)
        ...
        TGenerator=gt.GenTemplate(Tsample=args.time,fe=args.fe,kindTemplate=args.kindTemplate,whitening=args.white,verbose=args.verbose)
        TGenerator.majParams(args.m1,args.m2)
